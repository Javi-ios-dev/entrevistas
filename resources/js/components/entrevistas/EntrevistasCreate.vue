        ...
        <form @submit.prevent="saveEntrevista()">
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="relative z-0 w-full mb-6 group">
                    <label for="countries"
                        ...
                        el area</label>
                    <select v-model="form.vacante" class="form-control" id="docente">
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <label for="countries"
                        class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-8 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Selecciona
                        el prospecto</label>
                    <select v-model="form.prospecto" class="form-control" id="docente">
            ...
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="">
                    ...
                    <input type="date" v-model="form.fecha_entrevista" />
                </div>
            </div>
            <br>
            <div class="relative z-0 w-full mb-6 group">
                <textarea v-model="form.notas" type="text" name="floating_last_name" id="floating_last_name"
...
export default {
    setup() {
        const { errors, storeEntrevista, vacantes, getVacantes, prospectos, getProspectos } = useEntrevistas()
        const form = reactive({
            'vacante': '',
            'prospecto': '',
        ...
        const saveEntrevista = async () => {
            await storeEntrevista({ ...form })
        }

        onMounted(getProspectos(), getVacantes())
