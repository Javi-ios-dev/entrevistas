<template>
    <div class="overflow-hidden overflow-x-auto min-w-full align-middle sm:rounded-md">
        <h1 class="text-4xl font-extrabold">Editar Prospecto</h1>
        <br>
        <div v-if="errors">
        ...
        <form @submit.prevent="saveProspecto()">
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="relative z-0 w-full mb-6 group">
                    <input v-model="prospecto.nombre" type="text" name="floating_last_name" id="floating_last_name"
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <input v-model="prospecto.correo" type="text" name="floating_last_name" id="floating_last_name"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " required />
                    <label for="floating_last_name"
                        class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
            ...
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="">
                    ...
                    <!-- <VDatePicker :popover="popover"  v-model="prospecto.fecha_registro">
                    ...
                    <input type="date" v-model="prospecto.fecha_registro" />
                </div>
            </div>
            <br>
            <button type="submit"
...
export default {
    ...
    setup(props) {
        const { errors, getProspecto, prospecto, updateProspecto } = useProspectos()
        const saveProspecto = async () => {
            await updateProspecto(props.id)
        }

        onMounted(getProspecto(props.id))
        return {
            prospecto,
            errors,
            saveProspecto,
